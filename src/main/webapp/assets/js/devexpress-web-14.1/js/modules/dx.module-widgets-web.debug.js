...
if (!DevExpress.MOD_WIDGETS_WEB) {
            ...
            _supportedKeys: function() {
                ...
                return $.extend(this.callBase(), {
                        ...
                        rightArrow: function(e) {
                            ...
                            e.stopPropagation();
                            if (e.ctrlKey) {
                                if (this._currentColor.a > 0 && this.option("editAlphaChannel"))
                        ...
                        leftArrow: function(e) {
                            ...
                            e.stopPropagation();
                            if (e.ctrlKey) {
                                if (this._currentColor.a < 1 && this.option("editAlphaChannel"))
                                    updateAlphaScaleValue(getAlphaScaleStep())
                            }
    ...
    (function($, DX, undefined) {
        ...
        var dxMenuBase = ui.CollectionWidget.inherit({
                ...
                _itemMouseEnterHandler: function(e) {
                    ...
                    isItemDisabled ? this._removeFocusedItem() : this._resetFocusedItem($itemElement);
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxContextMenu", ui, ui.dxMenuBase.inherit({
            ...
            _getOverlayOptions: function() {
                var that = this,
                    ...
                    overlayOptions = {
                        ...
                        onPositioning: $.proxy(that._overlayPositioningActionHandler, that)
                    };
            ...
            _showSubmenu: function($item) {
                ...
                this.callBase($item);
                $submenuElement.length && this._showSubmenuGroup($submenuElement, $item)
            },
        ...
        DX.registerComponent("dxMenu", ui, ui.dxMenuBase.inherit({
            ...
            _createSubmenu: function(items, $rootItem) {
                var that = this,
                    ...
                    instance;
                $submenuContainer.addClass(DX_CONTEXT_MENU_CLASS).dxSubmenu(submenuOptions).appendTo($rootItem);
                instance = $submenuContainer.dxSubmenu('instance');
            ...
            _getSubmenuByRootElement: function($rootItem) {
                ...
                if ($rootItem) {
                    ...
                    if ($submenu.length > 0)
                        submenu = $submenu.dxSubmenu('instance')
                }
                return submenu
            },
            ...
            _itemMouseEnterHandler: function(e) {
                var that = this,
                    ...
                    $item = that._getItemElementByEventArgs(e),
    ...
    (function($, DX, undefined) {
        ...
        DX.registerComponent("dxTreeView", ui, ui.CollectionWidget.inherit({
            ...
            _attachDblclickToItem: function($item) {
                ...
                $item.off(eventName).on(eventName, function() {
                    var itemData = that._getItemData($item),
                        expanded = that._expandedGetter(itemData),
                        node = that._getNodeByKey(that._getItemKey(itemData));
                    that._updateExpandedState(itemData, !expanded);
                    that._updateExpandedItemsUI(itemData);
                    that._fireExpandedStateUpdatedEvt(!expanded, itemData, node)
                })
            },
            ...
            _renderToggleItemVisibilityIconClick: function($icon, itemData) {
                var eventName = events.addNamespace("dxclick", this.NAME),
                    that = this;
                var clickAction = that._createAction($.proxy(that._updateExpandedItemsUI, that, itemData));
                ...
                $icon.off(eventName).on(eventName, function(e) {
                    ...
                    if ($icon.hasClass(TOGGLE_ITEM_VISIBILITY_OPENED_CLASS)) {
                        that._updateExpandedState(itemData, true);
                        handleItemExpanded(e)
                    }
                    else {
                        that._updateExpandedState(itemData, false);
            ...
            _updateExpandedState: function(itemData, isExpanded) {
                ...
                var expandedItems = this.option("expandedItems"),
                    ...
                    that = this;
                that._expandedSetter(itemData, isExpanded);
                that._expandedSetter(node, isExpanded);
                if (isExpanded) {
            ...
            _updateExpandedItemsUI: function(itemData) {
                var $node = this.element().find("[" + DATA_ITEM_ID + "='" + this._getItemKey(itemData) + "']"),
            ...
            _fireExpandedStateUpdatedEvt: function(isExpanded, itemData, node) {
            ...
            _expandFocusedContainer: function() {
                ...
                var itemData = this._getItemData(this._$focusedItem.find(">." + ITEM_CLASS)),
                    node = this._getNodeByKey(this._getItemKey(itemData));
                this._updateExpandedState(itemData, true);
                this._fireExpandedStateUpdatedEvt(true, itemData, node)
            },
            _collapseFocusedContainer: function() {
                ...
                this._fireExpandedStateUpdatedEvt(false, itemData, node)
            },
            ...
            expandItem: function(itemElement) {
                ...
                var itemData = this._getItemData(itemElement),
                    node = this._getNodeByKey(this._getItemKey(itemData)),
                    expandedItems = this._enlargeExpandedItemsOption(itemData, this.option("expandedItems"));
                            ...
                            var that = this,
                                ...
                                updateSortGroupParameterIndexes = function(columns, sortParameters, indexParameterName) {
                                    ...
                                    $.each(columns, function(index, column) {
                                        if (column.dataField || column.name) {
                                            delete column[indexParameterName];
                                            if (sortParameters && sortParameters.length)
                                                for (i = 0; i < sortParameters.length; i++)
                                                    if (sortParameters[i].selector === column.dataField || sortParameters[i].selector === column.name) {
                ...
                views: {
                    ...
                    rowsView: {
                        ...
                        _rowClick: function(rowIndex, event) {
                            var that = this,
                                isCommandSelect = $(event.target).closest("." + DATAGRID_COMMAND_SELECT_CLASS).length,
                                isSelectionDisabled = $(event.target).closest("." + DATAGRID_SELECTION_DISABLED_CLASS).length;
                            if (!isCommandSelect) {
    ...
    (function($, DX) {
        ...
        var DATAGRID_CHECKBOX_SIZE_CLASS = "dx-datagrid-checkbox-size",
            ...
            ARROW_UP_KEY = 38,
            ARROW_DOWN_KEY = 40,
            UPDATE_VALUE_KEYS = [ENTER_KEY, TAB_KEY],
            UPDATE_NUMBER_VALUE_KEYS = [ENTER_KEY, TAB_KEY, ARROW_DOWN_KEY, ARROW_UP_KEY],
        ...
        dataGrid.EditorFactoryController = dataGrid.ViewController.inherit(function() {
                    ...
                    return getResultConfig({
                            ...
                            onValueChanged: function(e) {
                                var updateValue = function(e) {
                                    ...
                                    updateValueKeys = options._valueUpdateKeys || UPDATE_VALUE_KEYS;
                                ...
                                if (e.jQueryEvent && e.jQueryEvent.type === 'keyup')
                                    ...
                                    if ($.inArray(e.jQueryEvent.which, updateValueKeys) > -1)
                                        updateValue(e)
                                }
                                else
                                    updateValue(e)
            ...
            var createNumberBox = function($container, options) {
                    options._valueUpdateKeys = UPDATE_NUMBER_VALUE_KEYS;
                    var config = getTextEditorConfig($container, options);
            ...
            return {
                    _getFocusedElement: function($dataGridElement) {
                        return $dataGridElement.find('[tabindex]:focus:not(.dx-checkbox):not(.dx-menu):not(.dx-menu-item), input:focus')
                    },
                    ...
                    init: function() {
                        ...
                        $container && $container.on(addNamespace('keydown', DATAGRID_MODULE_NAMESPACE), function(e) {
                            if (e.which === 9)
        ...
        dataGrid.ColumnChooserView = dataGrid.ColumnsView.inherit({
            ...
            _initializePopupContainer: function() {
                ...
                if (!utils.isDefined(this._popupContainer)) {
                    that._popupContainer = $element.dxPopup(dxPopupOptions).dxPopup('instance');
        ...
        dataGrid.EditingController = dataGrid.ViewController.inherit(function() {
            ...
            return {
                    ...
                    editCell: function(rowIndex, columnIndex) {
                        ...
                        if (isCellEditingAllowed && item && item.rowType === 'data' && !item.removed && editMode !== DATAGRID_EDIT_MODE_ROW) {
                            ...
                            setTimeout(function() {
                                ...
                                $editCell && $editCell.find('[tabindex], input').focus();
                    ...
                    getColumnTemplate: function(options) {
                        ...
                        if (column.showEditorAlways && !utils.isDefined(column.groupIndex) || column.allowEditing && column.dataField && (isRowEditing || isCellEditing) && options.rowType === 'data') {
                ...
                views: {
                    rowsView: {
                        ...
                        _cellPrepared: function($cell, parameters) {
                            ...
                            if (!utils.isDefined(parameters.column.groupIndex) && (parameters.isEditing || parameters.column.showEditorAlways)) {
            ...
            extenders: {
                ...
                views: {rowsView: {
                        updateFreeSpaceRowHeight: function() {
                            var that = this,
                                $lastRowElement = that._getRowElements().last(),
                                $freeSpaceRowElement = that._getFreeSpaceRowElement(),
                                $tooltipContent = that.element().find(".dx-invalid-message .dx-overlay-content"),
                                $rowElement = $tooltipContent.closest("tr");
                            that.callBase();
                            if ($rowElement.is($lastRowElement) && $freeSpaceRowElement && (!$freeSpaceRowElement.is(":visible") || $tooltipContent.outerHeight() > $freeSpaceRowElement.outerHeight())) {
    ...
    (function($, DX) {
        ...
        dataGrid.DataController = dataGrid.Controller.inherit({}).include(ui.DataHelperMixin).inherit(function() {
            var members = {
                    ...
                    publicMethods: function() {
                        return ['beginCustomLoading', 'endCustomLoading', 'refresh', 'filter', 'clearFilter', 'combinedFilter', 'keyOf', 'byKey', 'getDataByKeys', 'pageIndex', 'pageSize', 'pageCount', 'totalCount', '_disposeDataSource', 'getKeyByRowIndex', 'getRowIndexByKey', 'isContentReady']
                    },
                    optionChanged: function(args) {
                        var that = this;
                        switch (args.name) {
                    ...
                    _handleCustomizeStoreLoadOptions: function(e) {
                        ...
                        var sort = columnsController.getSortDataSourceParameters();
                        if (sort) {
                            storeLoadOptions.sort = sort;
                            dataSource.sort(sort)
                        }
                        var group = columnsController.getGroupDataSourceParameters();
                        if (group) {
                            storeLoadOptions.group = group;
                    ...
                    _dataSourceOptions: function() {
                        var that = this,
                            ...
                            virtualMode = that.option('scrolling.mode') === 'virtual',
                            appendMode = that.option('scrolling.mode') === 'infinite';
                        return {
                                _preferSync: true,
        ...
        var GroupingRowsViewExtender = function() {
                return {
                        _rowClick: function(rowIndex, event) {
                            var that = this,
                                dataController = that.getController('data'),
                                $expandElement = $(event.target).closest('.' + DATAGRID_EXPAND_CLASS),
                                key;
                            that.callBase(rowIndex, event);
                ...
                return {
                        ...
                        _updateContentPosition: function() {
                            ...
                            if (virtualItemsCount) {
                                ...
                                if (that._contentHeight !== contentHeight || contentHeight === 0 || !!virtualTable.find("." + DATAGRID_COLUMN_LINES_CLASS).length !== that.option('showColumnLines') || $contentTable.find("col").length !== virtualTable.find("col").length) {
                ...
                _updateFilterOperationChooser: function($menu, column, $editorContainer) {
                    ...
                    $menu.dxMenu({
                        ...
                        onSubmenuHiding: function() {
                            $menu.blur();
                            delete $menu.dxMenu("instance")._$focusedItem;
        ...
        dataGrid.KeyboardNavigationController = dataGrid.ViewController.inherit({
            ...
            _tabKeyHandler: function(eventArgs, isEditing) {
                var editingOptions = this.option('editing'),
                    $cell;
                if (editingOptions && editingOptions.editEnabled && eventArgs.originalEvent.target && isEditing) {
                    this._updateFocusedCellPosition($(eventArgs.originalEvent.target).closest('td'));
